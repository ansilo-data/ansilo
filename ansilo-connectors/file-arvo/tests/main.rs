...
use ansilo_connectors_arvo::{
    ArvoConnector, ArvoConnectorEntitySourceConfig, ArvoDatabase, ArvoQuery,
    ArvoResultSet,
};
use ansilo_core::{
...
fn mock_data() -> (
    ConnectorEntityConfig<ArvoConnectorEntitySourceConfig>,
    ArvoDatabase,
) {
    let conf = ArvoDatabase::new();
    ...
    entities.add(EntitySource::new(
        ...
        ArvoConnectorEntitySourceConfig::default(),
    ));
...
#[test]
fn test_arvo_select_query_execution() {
    let (entities, data) = mock_data();

    let mut pool =
        ArvoConnector::create_connection_pool(data, &NodeConfig::default(), &entities).unwrap();
    ...
    let mut query = connection
        .prepare(ArvoQuery::new(select.into(), vec![]))
        .unwrap();
