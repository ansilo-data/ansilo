...
fn test() {
    ...
    // Should return zero rows when no file exists yet
    let rows = client
        .query_one(r#"SELECT COUNT(*) FROM "test_created_from_pg.avro""#, &[])
        .unwrap()
        .get::<_, i64>(0);
    ...
    let rows = client
        .execute(
            r#"
            INSERT INTO "test_created_from_pg.avro" (
                int,
                long,
                string,
                bool,
    ...
    let mut results = apache_avro::Reader::new(
        OpenOptions::new()
            .read(true)
            .open(avro_path.join("test_created_from_pg.avro"))
            .unwrap(),
    )
    .unwrap();
