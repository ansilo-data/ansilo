    ...
    fn start(&self, socket: UnixStream) -> Result<()> {
        ...
        let _ = thread::spawn(move || {
            ...
            match (pool, &*entities) {
                ...
                (ConnectionPools::Jdbc(pool), RwLockEntityConfigs::CdataJdbc(entities)) => {
                    Self::process::<CDataJdbcConnector>(auth, nc, chan, pool, entities, log)
                }
                (
...
pub enum RwLockEntityConfigs {
    ...
    CdataJdbc(
        RwLock<ConnectorEntityConfig<<CDataJdbcConnector as Connector>::TEntitySourceConfig>>,
    ),
    NativePostgres(
...
impl From<ConnectorEntityConfigs> for RwLockEntityConfigs {
    fn from(conf: ConnectorEntityConfigs) -> Self {
        match conf {
            ...
            ConnectorEntityConfigs::CdataJdbc(e) => Self::CdataJdbc(RwLock::new(e)),
